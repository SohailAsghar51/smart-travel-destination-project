...
# Columns: id, name, country, region, category, description, avg_cost_pkr, best_season, climate,
# safety_rating, rating, popularity_score, latitude, longitude, image_url, is_active
_DEST_SELECT = (
...
def list_destination_categories():
    """Distinct non-empty category values from destinations.category (lowercase, trimmed), sorted."""
    conn = create_database_connection()
    cursor = conn.cursor()
    cursor.execute(
...
def get_trip_detail_for_user(user_id, trip_id):
    """
    Returns trip header, cost breakdown, and day-by-day items for one trip owned by user_id.
    Shape matches the frontend plan view (itinerary array with items).
    """
    conn = create_database_connection()
    cursor = conn.cursor()
    try:
...
def _sql_time_value(s):
    """Accept '9:00', '09:30', '9:00 AM' (best effort) for MySQL TIME; None if unparseable."""
    if s is None or s == "":
        return None
...
def create_trip_with_itinerary(
    ...
    """day_plans: list of {day_number, day_title?, items: [{..., start_time, end_time, latitude, longitude}] }"""
    conn = create_database_connection()
    cursor = conn.cursor()
    try:
        cursor.execute(
